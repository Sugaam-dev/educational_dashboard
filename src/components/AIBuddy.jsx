...
How can I assist you with international governance decisions today?`,
  ...
  const parseMarkdown = (text) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Headers
      if (line.startsWith('### ')) {
        parsedHtml.push(`<h3 class="text-sm font-extrabold text-slate-800 mt-4 mb-2 uppercase tracking-wide flex items-center gap-1.5"><span class="h-1.5 w-1.5 rounded-full bg-blue-500"></span>${line.substring(4)}</h3>`);
      } else if (line.startsWith('#### ')) {
        parsedHtml.push(`<h4 class="text-xs font-bold text-slate-700 mt-3 mb-1.5 uppercase">${line.substring(5)}</h4>`);
      } 
      // Bold text block
      else if (line.startsWith('**') && line.endsWith('**')) {
        parsedHtml.push(`<p class="text-xs font-extrabold text-slate-900 mt-2">${line.substring(2, line.length - 2)}</p>`);
      } 
      // Bullet list items
        ...
        parsedHtml.push(`<li class="text-xs text-slate-600 mb-1.5 list-none pl-4 relative font-medium"><span class="absolute left-0 top-1.5 h-1.5 w-1.5 rounded-full bg-blue-500"></span>${content}</li>`);
      } 
      // Alert Callouts
      else if (line.startsWith('> [!')) {
        ...
        parsedHtml.push(`
          <div class="flex items-start gap-2.5 p-3.5 my-3 border rounded-xl text-xs font-medium ${alertClass}">
            <span>${nextLine}</span>
          </div>
        `);
      ...
      // Normal paragraphs
        ...
        content = content.replace(/_([^_]+)_/g, '<em>$1</em>');
        parsedHtml.push(`<p class="text-xs text-slate-600 leading-relaxed my-2">${content}</p>`);
      }
    }
  ...
  const renderTableHtml = (rows) => {
    ...
    let html = `<div class="overflow-x-auto my-3"><table class="w-full text-left border-collapse text-[11px] border border-slate-100 rounded-xl overflow-hidden">`;
    
    // Header
    html += `<thead><tr class="bg-slate-50 border-b border-slate-100">`;
    headers.forEach(h => {
      html += `<th class="py-2 px-3 font-bold text-[9px] text-slate-400 uppercase tracking-wide">${h}</th>`;
    });
    html += `</tr></thead>`;
  ...
  return (
    <div className="h-[calc(100vh-9.5rem)] flex flex-col xl:flex-row gap-6 animate-in fade-in duration-300">
      
      {/* Chat Conversation Area (3/4 width) */}
      <div className="flex-1 bg-white border border-slate-200 rounded-2xl shadow-sm flex flex-col justify-between overflow-hidden">
        ...
        <div className="p-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-3">
            <div className="h-9 w-9 rounded-xl bg-blue-600 flex items-center justify-center text-white shadow shadow-blue-500/20">
              <Sparkles className="h-4.5 w-4.5 text-white" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-slate-900">Conversational Decision Helper</h3>
              <span className="text-[10px] text-slate-400 font-bold uppercase">Online &bull; Connected to Registrar Databases</span>
            </div>
          </div>
          <button 
            onClick={() => setMessages([messages[0]])}
            className="text-[10px] font-bold text-slate-400 hover:text-slate-600 bg-white border border-slate-200 px-2.5 py-1 rounded-lg transition-colors flex items-center space-x-1"
          >
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4 bg-slate-50/20">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                {/* Sender Avatar */}
                <div className={`h-8 w-8 rounded-lg shrink-0 flex items-center justify-center text-xs font-bold shadow-sm ${
                  isBot 
          ...
          {isTyping && (
            <div className="flex gap-3 mr-auto max-w-md">
              <div className="h-8 w-8 rounded-lg bg-blue-600 text-white shrink-0 flex items-center justify-center">
                <Bot className="h-4 w-4" />
              </div>
              <div className="bg-white border border-slate-200/60 rounded-2xl p-4 shadow-sm flex items-center space-x-1">
        ...
        <div className="p-4 border-t border-slate-100 bg-white shrink-0">
          ...
          >
            <input
              ...
              placeholder="Ask about compliance risks, underperforming programs, visa issues..."
              className="flex-1 px-4 py-2.5 border border-slate-200 rounded-xl bg-slate-50 text-slate-700 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/25 focus:border-blue-500 transition-all font-semibold"
            />
            <button
              type="submit"
              disabled={!query.trim() || isTyping}
              className="px-4 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl text-xs font-bold transition-all shadow-md active:scale-95 flex items-center space-x-1 shrink-0"
            >
              <span>Query</span>
      ...
      {/* Suggested Queries Side panel (1/4 width) */}
      <div className="w-full xl:w-80 bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between shrink-0 overflow-y-auto">
        <div>
